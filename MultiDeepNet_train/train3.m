...
%% 5) Pré-alocação para métricas (evita warnings)
maxVal = ceil(numIterEp * numEpochs / valFreq);
f1_history = zeros(1, maxVal);
iter_history = zeros(1, maxVal);
valCount = 0;

%% 6) Treinamento com métricas
for epoch = 1:numEpochs
    ...
    for i = 1:numIterEp
        ...
        if mod(iteration,valFreq)==0
            ...
            valCount = valCount + 1;
            f1_history(valCount) = f1;
            iter_history(valCount) = iteration;
...
%% 8) Gráfico F1-score vs iteração
figure;
plot(iter_history(1:valCount), f1_history(1:valCount), 'b-', 'LineWidth', 2);
xlabel('Iteração');
ylabel('F1-score');
title('Evolução do F1-score na validação');
grid on;
