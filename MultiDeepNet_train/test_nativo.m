...
layers = [
    ...
    binaryCrossEntropyCustomLayer("bce")                       % camada de perda nativa
];

% -------------------------------------------------------------------------
% 4) Preparar rótulos no formato requerido (1×1×C×N) para BCE
% -------------------------------------------------------------------------
YTrain_4D = reshape(YTrain,1,1,numClasses,[]);
YVal_4D   = reshape(YVal  ,1,1,numClasses,[]);

%% 5) Opções de treinamento (infra-estrutura nativa do MATLAB)
%      β1, β2 e taxa de aprendizagem são iguais aos do artigo.
%      As demais escolhas são locais do script.
options = trainingOptions('adam', ...
    'InitialLearnRate',1e-4, ...                  % η
    'GradientDecayFactor',0.9, ...                % β1
    'SquaredGradientDecayFactor',0.999, ...       % β2
    'L2Regularization',1e-4, ...                  % weight decay global
    'MaxEpochs',100, ...                          % conforme artigo
    'MiniBatchSize',128, ...                      %% ─ NÃO ESPECIFICADO NO ARTIGO
    'Shuffle','every-epoch', ...
    'ValidationData',{XVal,YVal_4D}, ...
    'ValidationFrequency',200, ...                %% ─ NÃO ESPECIFICADO NO ARTIGO
    'ValidationPatience',10, ...                  %% ─ NÃO ESPECIFICADO NO ARTIGO
    'Plots','training-progress', ...
    'Verbose',true);

%% 6) Treinar rede
net = trainNetwork(XTrain, YTrain_4D, layers, options);

%% 7) Salvar a melhor rede encontrada
save coarseDOA_net.mat net                        %% ─ NÃO ESPECIFICADO
fprintf('Modelo salvo.\n');
