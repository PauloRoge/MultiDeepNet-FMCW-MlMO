...
layers = [
    ...
    % dropoutLayer(0.3)                              % regularização ─ adicionado
    % reluLayer
...
learnRate     = 1e-4;%3e-4;                % η  (ligeiramente menor p/ estabilidade)
gradDecay     = 0.9;                 % β1
sqDecay       = 0.999;               % β2
weightDecay   = 1e-4;                % L2 ─ regularização NÃO ESPECIFICADA
...
% Pré-alocar vetores de perda por avaliação periódica
maxLossPoints = ceil((numEpochs * numIterEp) / valFreq);
trainLossVec = nan(1, maxLossPoints);
valLossVec   = nan(1, maxLossPoints);
iterVec      = nan(1, maxLossPoints);
lossCount    = 0;
...
for epoch = 1:numEpochs
    ...
    for i = 1:numIterEp
        ...
        % Forward + back-prop; gradFun implementa BCE + L2
        [Ypred, grad, lossTr] = dlfeval(@gradFun, dlnet, Xb, Yb, weightDecay);
...
%% 7) Plotar curva de aprendizado (por iteração)
validIdx = 1:lossCount;
figure;
plot(iterVec(validIdx), trainLossVec(validIdx), '-b','LineWidth',1.5); hold on;
plot(iterVec(validIdx), valLossVec(validIdx),   '-r','LineWidth',1.5);
xlabel('iterations'); ylabel('Loss');
legend('trainLoss','valLoss'); grid on;
title('Curva de Perda por Iteração (a cada 200 iterações)');

% -------------------------------------------------------------------------
% Funções auxiliares
% -------------------------------------------------------------------------
function [YPred, gradients, loss] = gradFun(net, X, Y, weightDecay)
    % Forward
    YPred = forward(net, X);

    % Binary Cross-Entropy (formato classe-independente)
    lossBCE = -mean(Y .* log(YPred + eps) + ...
